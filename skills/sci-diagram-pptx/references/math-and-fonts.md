...
Write the chosen Latin, East Asian, and complex-script font mappings explicitly when the API exposes them. Set font size, paragraph alignment, line spacing, text-box margins, and meaningful line breaks during generation. Size the final text box before export and leave practical width and height headroom. Prefer enlarging the text region or making a small source-faithful line-break adjustment before reducing font size.
